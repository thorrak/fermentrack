...
class GravitySensor(models.Model):
    ...
    def retrieve_loggable_temp(self) -> (float or None, str or None):
        ...
        if point is None:
            return None, None
        elif point.temp is None:
            return None, None
        else:
            return round(point.temp, 1), point.temp_format
    ...
    @classmethod
    def from_dict(cls, input_dict, update=False) -> 'GravitySensor' or None:
        """
        Parse an input_dict generated by the to_dict function above, and return an instance of this class generated
        ...
        """
        try:
            sensor = GravitySensor.objects.get(uuid=input_dict['uuid'])
            if not update:
                return None
        except GravitySensor.DoesNotExist:
            sensor = GravitySensor()

        sensor.name = input_dict['name']
        sensor.sensor_type = input_dict['sensor_type']
        sensor.temp_format = input_dict['temp_format']
        sensor.log_gravity = input_dict['log_gravity']
...
class GravityLog(models.Model):
    ...
    @classmethod
    def from_dict(cls, input_dict, update=False) -> 'GravityLog' or None:
        """
        Parse an input_dict generated by the to_dict function above, and return an instance of this class generated
        using the inputs passed in input_dict.
        Start by querying the database to see if an object with input_dict.uuid exists. If one does, and update is False
        ...
        """
        try:
            log = cls.objects.get(uuid=input_dict['uuid'])
            if not update:
                return None
...
class TiltTempCalibrationPoint(models.Model):
    ...
    @classmethod
    def from_dict(cls, input_dict, update=False) -> 'TiltTempCalibrationPoint' or None:
        try:
            obj = cls.objects.get(uuid=input_dict['uuid'])
            if not update:
                # Return None if there is an existing object & we aren't supposed to update it
                return None
        except:
            obj = cls()
...
class TiltConfiguration(models.Model):
    ...
    def to_dict(self) -> dict:
        return {
            'color': self.color,
            'smoothing_window_vals': self.smoothing_window_vals,
            'polling_frequency': self.polling_frequency,
            'connection_type': self.connection_type,
            'grav_second_degree_coefficient': self.grav_second_degree_coefficient,
            'grav_first_degree_coefficient': self.grav_first_degree_coefficient,
            'grav_constant_term': self.grav_constant_term,
            'coefficients_up_to_date': self.coefficients_up_to_date,
            'sensor_uuid': str(self.sensor.uuid),
            'tiltbridge_uuid': str(self.tiltbridge.uuid) if self.tiltbridge else str(None),
            'uuid': str(self.uuid),
        }

    @classmethod
    def from_dict(cls, input_dict, update=False) -> 'TiltConfiguration' or None:
        """
        Parse an input_dict generated by the to_dict function above, and return an instance of this class generated
        using the inputs passed in input_dict.
        Start by querying the database to see if an object with input_dict.uuid exists. If one does, and update is False
        ...
        """
        try:
            tilt_config = cls.objects.get(uuid=input_dict['uuid'])
            if not update:
                return None
        except cls.DoesNotExist:
            tilt_config = cls()

        tilt_config.color = input_dict['color']
        tilt_config.smoothing_window_vals = input_dict['smoothing_window_vals']
        tilt_config.polling_frequency = input_dict['polling_frequency']
        tilt_config.connection_type = input_dict['connection_type']
        tilt_config.grav_second_degree_coefficient = input_dict['grav_second_degree_coefficient']
        tilt_config.grav_first_degree_coefficient = input_dict['grav_first_degree_coefficient']
...
class TiltBridge(models.Model):
    ...
    def to_dict(self) -> dict:
        return {
            'name': self.name,
            'mdns_id': self.mdns_id,
            'uuid': str(self.uuid),
        }

    @classmethod
    def from_dict(cls, input_dict, update=False) -> 'TiltBridge' or None:
        """
        Parse an input_dict generated by the to_dict function above, and return an instance of this class generated
        using the inputs passed in input_dict.
        Start by querying the database to see if an object with input_dict.uuid exists. If one does, and update is False
        ...
        """
        try:
            tiltbridge = cls.objects.get(uuid=input_dict['uuid'])
            if not update:
                return None
        except cls.DoesNotExist:
            tiltbridge = cls()

        tiltbridge.name = input_dict['name']
        tiltbridge.mdns_id = input_dict['mdns_id']
        tiltbridge.uuid = input_dict['uuid']
...
class IspindelGravityCalibrationPoint(models.Model):
    ...
    @classmethod
    def from_dict(cls, input_dict, update=False) -> 'IspindelGravityCalibrationPoint' or None:
        """
        Parse an input_dict generated by the to_dict function above, and return an instance of this class generated
        using the inputs passed in input_dict.
        Start by querying the database to see if an object with input_dict.uuid exists. If one does, and update is False
        ...
        """
        try:
            calibration_point = cls.objects.get(uuid=input_dict['uuid'])
            if not update:
                return None
        except cls.DoesNotExist:
            calibration_point = cls()
...
class IspindelConfiguration(models.Model):
    ...
    def to_dict(self) -> dict:
        # Return the object as a dict, so we can easily convert it to JSON later
        return {
            'sensor_id': self.sensor_id,
            'name_on_device': self.name_on_device,
            'third_degree_coefficient': self.third_degree_coefficient,
            'second_degree_coefficient': self.second_degree_coefficient,
            'first_degree_coefficient': self.first_degree_coefficient,
